...
const quickActions: QuickAction[] = [
  ...
  { id: 4, title: 'Create poll', icon: 'chart-bar', color: 'primary' },
];
...
export default function HomeScreen({ userAvatar, createdProfile, navigation }: HomeScreenProps & { createdProfile?: any, navigation?: any }) {
  ...
  const onPanStateChange = (event: any) => {
    if (event.nativeEvent.state === State.END) {
      if (Math.abs(event.nativeEvent.translationY) > 120) {
        runOnJS(setImageModalVisible)(false);
        scale.value = 1;
        translateY.value = 0;
      } else {
    ...
    switch (action.title) {
      ...
      case 'Add video':
        ...
        if (navigation) {
          ...
          Alert.alert('Post Screen', `Opening post screen for: ${action.title}`);
        }
        break;
      case 'Create poll':
        ...
        break;
      default:
        Alert.alert('Quick Action', `You selected: ${action.title}`);
    }
  };
  ...
  return (
    <View style={[styles.container, { backgroundColor: theme.backgroundColor }]}> 
      ...
      <Menu
        ...
        options={[
          { label: 'Edit', icon: 'pencil', onPress: () => { if (menuPost) Alert.alert('Edit', `Edit post ${menuPost.id}`); } },
          { label: 'Delete', icon: 'delete', onPress: () => { if (menuPost) Alert.alert('Delete', `Delete post ${menuPost.id}`); } },
          { label: 'Report', icon: 'alert-circle-outline', onPress: () => { if (menuPost) Alert.alert('Report', `Report post ${menuPost.id}`); } },
        ]}
